...
use super::{
    ...
    parse::{invalid_moc3, read_i32_section, to_usize},
};
    ...
    pub fn parse(bytes: &[u8]) -> Result<Self> {
            ...
            .max()
        {
            Some(index) => usize::try_from(index)
                ...
                .ok_or_else(|| invalid_moc3("offscreen count is too large"))?,
            None => 0,
        };
        if offscreen_count > part_count {
            return Err(invalid_moc3(format!(
                "offscreen count {offscreen_count} exceeds part count {part_count}"
            )));
        }
