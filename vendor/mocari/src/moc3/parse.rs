...
pub(super) fn read_i32_section_or_default(
    ...
    match offsets.section_offset(slot) {
        Some(0) | None => default_section(count, default, slot),
...
pub(super) fn read_section<T>(
    ...
    values.try_reserve_exact(count).map_err(|error| {
        invalid_moc3(format!(
            "section slot {slot} cannot reserve {count} elements: {error}"
        ))
    })?;
...
fn default_section<T: Clone>(count: usize, default: T, slot: usize) -> Result<Vec<T>> {
    let mut values = Vec::new();
    values.try_reserve_exact(count).map_err(|error| {
        invalid_moc3(format!(
            "default section slot {slot} cannot reserve {count} elements: {error}"
        ))
    })?;
