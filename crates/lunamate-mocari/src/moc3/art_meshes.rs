...
const MAX_ART_MESH_COUNT: usize = 4_096;
const MAX_TOTAL_ART_MESH_VERTICES: usize = 1_000_000;
const MAX_TOTAL_ART_MESH_INDICES: usize = 3_000_000;
const MAX_MASKS_PER_ART_MESH: usize = 32;
const MAX_TOTAL_ART_MESH_MASKS: usize = MAX_ART_MESH_COUNT * MAX_MASKS_PER_ART_MESH;
...
impl Moc3ArtMeshes {
    ...
    pub fn from_parts_with_hierarchy_and_render_orders(
        ...
        if meshes.len() > MAX_ART_MESH_COUNT {
            return Err(invalid_moc3(format!(
                "art mesh count {} exceeds limit {MAX_ART_MESH_COUNT}",
                meshes.len()
            )));
        }
        ...
        for (index, mesh) in meshes.iter().copied().enumerate() {
            let vertex_count = usize::try_from(mesh.vertex_count())
                .map_err(|_| invalid_moc3(format!("art mesh {index} vertex count is negative")))?;
            let index_count = usize::try_from(mesh.position_index_count())
                .map_err(|_| invalid_moc3(format!("art mesh {index} index count is negative")))?;
            let mask_count = usize::try_from(mesh.mask_count())
                .map_err(|_| invalid_moc3(format!("art mesh {index} mask count is negative")))?;
            if mask_count > MAX_MASKS_PER_ART_MESH {
                return Err(invalid_moc3(format!(
                    "art mesh {index} mask count {mask_count} exceeds limit {MAX_MASKS_PER_ART_MESH}"
                )));
            }
            total_vertices = total_vertices
                .checked_add(vertex_count)
                .ok_or_else(|| invalid_moc3("total art mesh vertex count overflows"))?;
            total_indices = total_indices
                .checked_add(index_count)
                .ok_or_else(|| invalid_moc3("total art mesh index count overflows"))?;
            total_masks = total_masks
                .checked_add(mask_count)
                .ok_or_else(|| invalid_moc3("total art mesh mask count overflows"))?;
            if total_vertices > MAX_TOTAL_ART_MESH_VERTICES
                || total_indices > MAX_TOTAL_ART_MESH_INDICES
                || total_masks > MAX_TOTAL_ART_MESH_MASKS
            {
                return Err(invalid_moc3("art mesh runtime geometry budget exceeded"));
            }
