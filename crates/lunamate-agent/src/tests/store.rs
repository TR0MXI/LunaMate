...
use crate::{
    ...
    persistence::PersistentMemoryTier as MemoryTier,
...
use super::persistence::TestDatabase as Database;
...
fn missing_session_returns_default() {
    run_async(async {
        ...
        let (session, store) =
            ChatSessionStore::load(database.callbacks(), PERSONA, ChatLimits::default())
                .await
                .expect("缺失会话应降级为空会话");
...
fn session_round_trip_uses_database() {
    run_async(async {
        ...
        let store = ChatSessionStore::empty(database.callbacks());
        store
...
fn tombstoned_persona_cleanup_removes_session_and_all_memory_tiers() {
    run_async(async {
        ...
        let (_, store) =
            ChatSessionStore::load(database.callbacks(), "deleted", ChatLimits::default())
                .await
...
fn corrupt_database_session_returns_error() {
    run_async(async {
        ...
        let result =
            ChatSessionStore::load(database.callbacks(), PERSONA, ChatLimits::default()).await;
...
fn availability_reflects_whether_a_database_was_opened() {
    run_async(async {
        ...
        assert!(ChatSessionStore::empty(database.callbacks()).is_available());
...
fn store_errors_describe_the_failure_without_including_session_text() {
    ...
    assert!(unavailable.to_string().contains("持久化"));
...
fn deleting_a_persona_session_removes_only_that_personas_document() {
    run_async(async {
        ...
        delete_persona_session(&database.callbacks(), "default")
            .await
            .expect("删除人格会话应成功");

        let (cleared, _) =
            ChatSessionStore::load(database.callbacks(), "default", ChatLimits::default())
                .await
                .expect("删除后应降级为空会话");
        assert_eq!(cleared.messages().len(), 0);
        // 记忆按人格隔离，删除一个人格不得影响其他人格的上下文。
        let (kept, _) =
            ChatSessionStore::load(database.callbacks(), "other", ChatLimits::default())
                .await
                .expect("另一人格的会话应仍可恢复");
...
fn stored_context_usage_reports_zero_for_a_persona_without_history() {
    run_async(async {
        ...
        let usage =
            crate::store::persona_context_usage(&database.callbacks(), &lock, "fresh", limits)
                .await
                .expect("没有记录时应返回零占用");
...
fn concurrent_non_active_edits_are_serialized_without_losing_each_other() {
    run_async(async {
        ...
        let (_, store) =
            ChatSessionStore::load_with_lock(database.callbacks(), PERSONA, limits, lock.clone())
                .await
                .expect("空会话应可加载");
        store
            .save(session.snapshot(1))
        ...
        let callbacks = database.callbacks();
        let first = mutate_persona_session(&callbacks, &lock, PERSONA, limits, |session| {
            session.edit_message(user_id, "edited question")?;
            Ok(true)
        });
        let second = mutate_persona_session(&callbacks, &lock, PERSONA, limits, |session| {
        ...
        let (restored, _) =
            ChatSessionStore::load_with_lock(database.callbacks(), PERSONA, limits, lock)
                .await
                .expect("编辑后的会话应可恢复");
...
fn cancelling_a_reserved_waiter_does_not_block_later_operations() {
    run_async(async {
        let database = Database::open_memory().await.expect("内存数据库应可打开");
        let lock: SessionDocumentLock = Arc::new(SessionDocumentCoordinator::new());
        let (_, store) = ChatSessionStore::load_with_lock(
            database.callbacks(),
            PERSONA,
            ChatLimits::default(),
            lock,
