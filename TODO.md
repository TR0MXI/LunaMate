...
- [x] 提供关闭、自动和快捷键按住说话三种模式；自动模式允许快捷键接管 VAD 录音，STT 列表
  选中项与每个本地模型的 GPU、目标语言偏好原子持久化，未选择模型时不启用语音输入。
- [x] 在 `lunamate-agent` 中隔离 STT 与 TTS 协议，支持 OpenAI 和豆包云端转写与合成，并把
...
- [ ] 使用真实 OpenAI 与豆包凭据验证 STT/TTS 鉴权、限流、超时、音色、长回复和默认输出设备切换。
