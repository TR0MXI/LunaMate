...
- [x] 在供应商高级设置中提供思考强度、输出长度上限、temperature 与 top_p；预填默认值，
  未显式启用的参数不随请求发送。
- [x] 用单色品牌图标区分供应商条目，并记录来源、许可证与商标归属。
- [x] 实现可停止、可拒绝迟到结果并按批次刷新 UI 的流式文本对话。
- [x] 在桌宠窗口提供聊天开关、底部单行输入栏，以及可滚动并在终态后渐隐的主题回复层。
- [x] 按人格保存有界会话，并在重启后把未完成回复恢复为中断状态；切换人格时先落盘旧
  上下文再换入新上下文，换入完成前拒绝新消息。
...
- [x] 在人格设置页展示三层记忆用量，提供分层与整体清除入口，并对不可逆删除做二次确认。
- [x] 删除人格时同步删除其会话文档与全部 `agent_memory` 记录。
