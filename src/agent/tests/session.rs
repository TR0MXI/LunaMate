use crate::{
    agent::{
        Agent, AssistantTrace, ToolExecutionTrace, chat_limits, media::prepare_dynamic_image,
        session::*,
    },
    config::{
...
fn unavailable_persistence_keeps_the_active_personas_context_limits() {
    ...
    let (session, store, _, _) = Agent::without_persistence("offline".to_owned(), limits);

    assert_eq!(session.usage().max_messages, 2);
    assert_eq!(session.usage().max_tokens, 12);
...
fn voice_interruption_truncates_utf8_at_a_character_boundary() {
    let marker_tokens = context_message_tokens(VOICE_INTERRUPTION_MARKER, 4);
    let mut session = ChatSession::new(ChatLimits {
        max_messages: 4,
        max_tokens: marker_tokens
            + context_message_tokens("问题", 4)
            + estimate_text_tokens("回答回答")
            - 1,
...
fn next_request_keeps_interruption_semantics_after_history_trimming() {
    let mut session = ChatSession::new(ChatLimits {
        max_messages: 4,
        max_tokens: context_message_tokens(VOICE_INTERRUPTION_MARKER, 4) + 8,
        max_request_tokens: context_message_tokens(VOICE_INTERRUPTION_MARKER, 4) + 8,
    })
    .expect("测试限制应当有效");
...
#[test]
fn an_older_voice_marker_cannot_mask_the_latest_unmarked_interruption() {
    let mut session = ChatSession::default();
    ...
    assert!(
        session.messages()[1]
            .content()
            .ends_with(VOICE_INTERRUPTION_MARKER)
    );

    let second = session.start_turn("second").expect("第二轮应可开始");
    session
    ...
    session
        ...
        .expect("恢复默认预算应成功");
    let third = session.start_turn("third").expect("第三轮应可开始");
    assert!(
        third
            .context
            .last()
