...
use crate::agent::{ContextMessageRole, MemoryScope, palette::AgentPalette};

use super::{
    components::{confirm_overlay, form_field, section_label, status_toast},
    persona::{ContextMessageEditor, PersonaPage, PersonaSettingsView},
...
impl PersonaSettingsView {
    ...
    fn render_context_messages(&self, palette: AgentPalette, cx: &mut Context<Self>) -> AnyElement {
    ...
    fn render_context_message(
        ...
        let assistant = message.role == ContextMessageRole::Assistant;
...
fn message_role_icon(icon: &'static str, palette: AgentPalette) -> AnyElement {
    div()
