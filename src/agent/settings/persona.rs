...
use crate::{
    agent::{
        AgentMemoryAccess, ContextMessage, ContextMessageRole, ContextUsage, MemoryScope,
        PersonaMemoryUsage, chat_limits,
        session::{MAX_SESSION_TEXT_BYTES, context_message_tokens},
    },
    config::{
        CONFIG, CONTEXT_MESSAGES_MAX, CONTEXT_MESSAGES_MIN, CONTEXT_TOKENS_MAX, CONTEXT_TOKENS_MIN,
...
#[derive(Clone)]
pub(crate) struct PersonaSettingsDraft {
...
impl PersonaSettingsDraft {
    ...
    pub(crate) fn prepare_write(&self) -> Option<PersonaSettingsDraftWrite> {
    ...
    pub(crate) fn finish_persona_cleanup(&mut self, persona: &str) -> bool {
    ...
    pub(crate) fn persona_cleanup_was_published(&mut self, persona: &str) {
...
/// 不暴露人格配置类型的退出边界写入。
pub(crate) struct PersonaSettingsDraftWrite {
    settings: PersonaSettings,
    revision: u64,
}

impl PersonaSettingsDraftWrite {
    pub(crate) fn persist(self) -> Result<bool, String> {
        CONFIG
            .set_persona_settings_at_revision(self.settings, self.revision)
...
pub(super) struct ContextMessageEditor {
    pub(super) id: u64,
    pub(super) role: ContextMessageRole,
...
impl PersonaSettingsView {
    ...
    pub(crate) fn new(
        ...
        // 转移草稿可能来自一次失败或尚未完成的窗口写入；只能把已发布配置视为已提交。
    ...
    pub(crate) fn refresh_live2d_models(
        &mut self,
    ...
    fn clear_memory(
        ...
        let track = cx.spawn(async move |this, cx| {
            let stored_result = match task.await {
                Ok(Ok(())) => Ok(()),
    ...
    pub(super) fn save(&mut self, cx: &mut Context<Self>) {
        ...
        let task = cx.spawn(async move |this, cx| {
            let result = background
                .spawn(async move {
                    CONFIG.set_persona_settings_at_revision(normalized, config_revision)
                })
                .await;
            let _ = this.update(cx, |this, cx| {
    ...
    pub(crate) fn resume_pending_work(&mut self, cx: &mut Context<Self>) {
        self.save(cx);
    }

    /// 合并旧窗口完成的清理结果，只修改当前草稿中的 tombstone。
    pub(crate) fn finish_persona_cleanup(&mut self, persona: &str, cx: &mut Context<Self>) {
    ...
    pub(crate) fn persona_cleanup_was_published(&mut self, persona: &str, cx: &mut Context<Self>) {
        self.draft
            .pending_deletions
            .retain(|pending| pending != persona);
    ...
    #[cfg(test)]
    pub(in crate::agent) fn confirm_message_for_test(&self) -> Option<String> {
        self.confirm_prompt().map(|prompt| prompt.message)
    }

    /// 直接创建单条上下文删除确认，跳过异步上下文加载。
    #[cfg(test)]
    pub(in crate::agent) fn request_delete_context_confirmation_for_test(
        &mut self,
        message_id: u64,
        cx: &mut Context<Self>,
    ) {
    ...
    #[cfg(test)]
    pub(in crate::agent) fn delete_persona_for_test(
        &mut self,
        persona: &str,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    #[cfg(test)]
    pub(in crate::agent) fn pending_deletions_for_test(&self) -> &[String] {
    ...
    #[cfg(test)]
    pub(in crate::agent) fn bound_live2d_for_test(&self, index: usize) -> Option<PathBuf> {
        self.draft
            .personas
            .get(index)
    ...
    #[cfg(test)]
    pub(in crate::agent) fn load_context_messages_for_test(
        &mut self,
        messages: Vec<ContextMessage>,
        window: &mut Window,
        cx: &mut Context<Self>,
    ...
    #[cfg(test)]
    pub(in crate::agent) fn show_context_for_test(&mut self, cx: &mut Context<Self>) {
    ...
    /// 返回当前已构造的消息编辑器数量。
    #[cfg(test)]
    pub(in crate::agent) fn context_editor_count_for_test(&self) -> usize {
        self.context_editors.len()
    }

    /// 返回当前卡片顺序，供测试断言拖拽使用稳定消息 ID。
    #[cfg(test)]
    pub(in crate::agent) fn context_message_ids_for_test(&self) -> Vec<u64> {
        self.context_editors
            .iter()
            .map(|message| message.id)
            .collect()
    ...
    /// 切换一条上下文消息的多选状态。
    #[cfg(test)]
    pub(in crate::agent) fn toggle_context_message_selected_for_test(
        &mut self,
        message_id: u64,
        cx: &mut Context<Self>,
    ) {
    ...
    /// 请求删除当前多选集合，但不进行确认。
    #[cfg(test)]
    pub(in crate::agent) fn request_delete_selected_context_messages_for_test(
        &mut self,
        cx: &mut Context<Self>,
    ) {
    ...
    /// 返回当前多选消息数量。
    #[cfg(test)]
    pub(in crate::agent) fn selected_context_messages_for_test(&self) -> usize {
        self.context_selected.len()
    }

    /// 复制当前选择，供无头测试验证多选顺序与剪贴板内容。
    #[cfg(test)]
    pub(in crate::agent) fn copy_selected_context_messages_for_test(
        &mut self,
        cx: &mut Context<Self>,
    ) {
    ...
    #[cfg(test)]
    pub(in crate::agent) fn context_scroll_for_test(&self) -> (gpui::Pixels, gpui::Pixels) {
        (
    ...
    #[cfg(test)]
    pub(in crate::agent) fn context_limit_inputs_for_test(
        &self,
        cx: &Context<Self>,
    ) -> [String; 2] {
        [
    ...
    #[cfg(test)]
    pub(in crate::agent) fn context_limits_for_test(
        &self,
        cx: &Context<Self>,
    ) -> PersonaContextLimits {
