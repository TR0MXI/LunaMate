...
use super::{GpuUnderlaySize, SurfaceSeed, WorkerMailbox};
use gpui::Window;
use gpui_wgpu::wgpu;
...
impl SurfaceFactory {
    ...
    pub(super) fn create(self, mailbox: &WorkerMailbox) -> Result<SurfaceSeed, String> {
        let (owner, child_ptr) = SurfaceOwner::new(self.display, self.parent, mailbox)?;
...
pub(super) struct SurfaceOwner {
    ...
    last_size: Option<GpuUnderlaySize>,
}
...
impl SurfaceOwner {
    fn new(
        ...
        while !state.initial_sync_done {
            ...
            if mailbox.wait_for_shutdown(REGISTRY_POLL_INTERVAL) {
    ...
    pub(super) fn prepare_present(&mut self, size: GpuUnderlaySize) -> Result<(), String> {
...
fn exact_buffer_scale(physical: u32, logical: u32) -> Option<i32> {
