...
const MAX_MASK_CONTEXTS: usize = 32;
...
impl CpuRenderer {
    ...
    fn update_draw_order(&mut self, meshes: &[Moc3DrawableMesh]) {
...
fn sorted_mask_sources(indices: &[i32]) -> Result<Vec<usize>, RenderError> {
...
pub(super) fn validate_gpu_model(
    meshes: &[Moc3DrawableMesh],
    textures: &[DecodedTexture],
    width: u32,
    height: u32,
...
pub(super) fn validate_gpu_frame(
    meshes: &[Moc3DrawableMesh],
    cancellation: &RenderCancellation,
) -> Result<(), RenderError> {
    for (drawable_index, drawable) in meshes.iter().enumerate() {
...
#[cfg(test)]
mod tests {
    use mocari::moc3::Moc3DrawableVertex;
