...
use crate::{
    ...
    chat::ChatView,
    config::{CONFIG, ConfigEvent, ConfigView, ConfigWindow, ConfigWindowView, ModelWindowSize},
    frame_scheduler::{FrameRateMeter, FrameWake},
    gpu_underlay::{GpuUnderlay, GpuUnderlayEvent, GpuUnderlaySize},
    interaction::{ModelCommand, ModelCommandSender, RenderedModelFrame},
    live2d_image::{ModelPreviewCapabilities, RenderCancellation},
    platform_window::{WindowMover, WindowPositionController, configure_settings_window},
    window::{
...
impl ModelLoadState {
    fn ready(diagnostics: ModelLoadDiagnostics) -> Self {
    ...
    fn diagnostics_message(&self) -> Option<SharedString> {
...
impl ModelView {
    ...
    pub(super) fn new(
        ...
        let config_subscription =
            cx.subscribe(&config, |this, _, event: &ConfigEvent, cx| match event {
                ...
                ConfigEvent::ShowFpsChanged(show) => this.set_show_fps(*show, cx),
                ConfigEvent::ModelWindowSizeChanged(size) => {
                    this.pending_model_window_size = Some(*size);
                    cx.notify();
                }
                ConfigEvent::PreviewMotion(group) => {
                ...
                ConfigEvent::PreviewExpression(name) => {
                    if let Some(sender) = &this.model_commands
                        && sender
                ...
                ConfigEvent::LlmChanged(settings) => {
                    this.chat.update(cx, |chat, cx| {
                        chat.update_settings(settings.clone(), cx);
                    });
                }
                ConfigEvent::WindowPositionsReset => {
                    this.position_controller.request_reset();
                    cx.notify();
                }
                ConfigEvent::AppearanceChanged(settings) => {
                    crate::theme::apply_language(settings.language);
                    this.model_state.refresh_localized_warning();
                    crate::theme::apply(settings, None, cx);
                }
            });
        ...
        let appearance_subscription = window.observe_window_appearance(|window, cx| {
            ...
            if appearance.theme == crate::theme::ThemePreset::System {
    ...
    pub(super) fn shutdown_gpu_for_quit(&mut self) {
        self.gpu_shutdown_restart_cpu = false;
        if self.gpu_shutdown_pending {
    ...
    fn toggle_chat(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.chat_open {
            ...
            self.chat.update(cx, |chat, cx| {
                chat.update_settings(CONFIG.llm_settings(), cx);
...
mod tests {
    ...
    fn ready_state_keeps_partial_capability_failures_non_fatal() {
        use crate::capabilities::{ModelDiagnosticCategory, ModelLoadDiagnostic};
