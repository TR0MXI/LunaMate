...
fn run(commands: std::sync::mpsc::Receiver<PlaybackCommand>) {
    ...
    loop {
        ...
        match command {
            PlaybackCommand::Play {
                samples,
                sample_rate,
            } => match build_output_stream(samples, sample_rate) {
                Ok((next, completed)) => {
                    _stream = Some(next);
...
fn build_output_stream(
    ...
    let stream = match supported.sample_format() {
        SampleFormat::I8 => build_typed::<i8>(&device, &config, channels, queue, completed.clone()),
        SampleFormat::I16 => {
            build_typed::<i16>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::I24 => {
            build_typed::<cpal::I24>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::I32 => {
            build_typed::<i32>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::I64 => {
            build_typed::<i64>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::U8 => build_typed::<u8>(&device, &config, channels, queue, completed.clone()),
        SampleFormat::U16 => {
            build_typed::<u16>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::U24 => {
            build_typed::<cpal::U24>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::U32 => {
            build_typed::<u32>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::U64 => {
            build_typed::<u64>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::F32 => {
            build_typed::<f32>(&device, &config, channels, queue, completed.clone())
        }
        SampleFormat::F64 => {
            build_typed::<f64>(&device, &config, channels, queue, completed.clone())
        }
