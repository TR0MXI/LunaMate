...
impl AuxiliaryResourceBudget {
    #[cfg(test)]
    pub(crate) fn with_limit(maximum_bytes: u64) -> Self {
        Self {
...
impl ModelResourceResolver {
    ...
    #[cfg(test)]
    pub(crate) fn read_text_with_budget(
        &self,
        reference: &str,
        maximum_bytes: u64,
...
mod tests {
    ...
    fn rejects_parent_reference_before_reading_file() {
        ...
        let resolver =
            ModelResourceResolver::for_manifest(&directory.path().join("model.model3.json"))
                .expect("测试模型目录应当可以解析");
    ...
    fn rejects_directory_and_oversized_optional_resource() {
        let directory = TestDirectory::new();
        fs::create_dir(directory.path().join("directory.exp3.json"))
            .expect("测试子目录应当可以创建");
        fs::write(directory.path().join("large.exp3.json"), "{}")
            .expect("测试超限资源应当可以创建");
        let resolver =
            ModelResourceResolver::for_manifest(&directory.path().join("model.model3.json"))
                .expect("测试模型目录应当可以解析");
